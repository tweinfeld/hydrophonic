...
const DOCKER_ENGINE_API_VERSION = "1.37";
const HTTP_RESPONSE = "http";
const TCP_RESPONSE = "tcp";

const isOkHttpHeader = (statusCode) => ~~(statusCode / 100) === 2;
const toArgBoolean = (bol) => (bol ? "1" : "0");
const serializeEnv = (arr) => _.map(arr, (value, key) => `${key}=${value}`);
const httpResponseStreamer = ($stream) => {
  return $stream.flatMap(({ req, res }) => {
    let stream = kefir
      .merge([
        kefir.constant({
          type: "header",
    ...
    return !isOkHttpHeader(res.statusCode)
      ? stream
          .thru(decodeResponse)
          .map((err) => ({
            status_code: res.statusCode,
...
const tcpResponseStreamer = ($stream) => {
  return $stream.flatMap(({ res, socket }) => {
...
const bufferResponse = ($res) =>
  ...
  decodeResponse = (function (decoders) {
        ...
        .combine(
          [
            $res
              ...
              .map(_.property("headers.content-type")),
            bufferResponse($res),
          ],
          (contentType, body) =>
            (
              decoders.find(({ test }) => test(contentType)) || {
                decode: _.identity,
              }
            ).decode(body),
        )
    ...
    {
      test: (contentType) => /^application\/json/i.test(contentType),
      decode: (body) => JSON.parse(body.toString("utf8")),
    },
  ]);

module.exports = function ({ ca, cert, key, url: baseUrl, socketPath } = {}) {
  const client = function ({
    ...
    return kefir.later().flatMap(() => {
      const req = (socketPath ? http : https).request({
        ...(function () {
          return baseUrl
            ? url.parse(
                [
                  [baseUrl, `v${DOCKER_ENGINE_API_VERSION}`, path].join("/"),
            ...
            : {
                socketPath,
                path: [
                  [`/v${DOCKER_ENGINE_API_VERSION}`, path].join("/"),
                  qs.stringify(queryString),
                ]
                  .filter(Boolean)
                  .join("?"),
      ...
      return kefir
        .merge([
          ...
          response === TCP_RESPONSE
            ? kefir.fromEvents(req, "upgrade", (res, socket) => ({
                req,
                res,
                socket,
              }))
  ...
  return {
    ...
    getImageList: () =>
      client({ path: "images/json" })
        .thru(decodeResponse)
        .map(fp.map(fp.property("Id")))
        .toProperty(),

    getContainerList: () =>
      client({ path: "containers/json" })
        .thru(decodeResponse)
        .map(fp.map(fp.property("Id")))
        .toProperty(),

    getImage: ({ id }) =>
      client({ path: `images/${id}/json` }).thru(decodeResponse),
      ...
      return client({
        ...
        .map(
          fp.pipe(
            ...
            fp.split(/\r?\n/),
            fp.compact,
            fp.map((line) => _.attempt(() => JSON.parse(line))),
          ),
        )
        .flatten()
        .filter(
        ...
        .scan((progressAcc, progressObj) => {
          let [id, type, current, total] = _.at(progressObj, [
            "id",
            "status",
          ...
          return _.merge(
            progressAcc || {},
            type === "Downloading" && { [id]: { current, total } },
          );
        })
        .map((downloadProgress) => {
          ...
          return {
            download_progress:
              allProgress.map(_.property("current")).reduce(_.add, 0) /
              allProgress.map(_.property("total")).reduce(_.add, 0),
          };
        });
    },
    ...
    startContainer: ({ id }) =>
      client({ path: `containers/${id}/start`, method: "POST" })
        .filter(_.matchesProperty("type", "header"))
        .map(({ status_code: statusCode }) => isOkHttpHeader(statusCode)),

    removeContainer: ({ id, force = true, removeVolumes = true }) =>
      client({
        path: `containers/${id}`,
        method: "DELETE",
        qs: { force, v: removeVolumes },
      }).thru(decodeResponse),
    ...
    attachContainer: ({ id, stdout = true, stderr = true }) => {
      return client({
        ...
        qs: {
          stream: "1",
          stdout: toArgBoolean(stdout),
          stderr: toArgBoolean(stderr),
        },
      });
    },
    ...
    buildImage: ({
      ...
      return {
        ...
        output: client({
          ...
          .flatten()
          .flatMap((logMessage) =>
            kefir[logMessage["error"] ? "constantError" : "constant"](
              logMessage,
            ),
          ),
      };
